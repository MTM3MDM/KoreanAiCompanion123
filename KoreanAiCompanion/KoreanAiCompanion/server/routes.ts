...
const groq = new Groq({
  apiKey: "gsk_0123456789abcdef", // 여기에 실제 GROQ API 키를 입력하세요
});

// Helper functions - 토키사키 쿠루미 AI 챗봇
export async function chatWithKurumi(text: string, conversationHistory: any[] = []): Promise<{response: string, confidence: number, contextAnalysis: string}> {
  try {
